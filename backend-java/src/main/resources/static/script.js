...
function updateUI() {
    ...
    try {
        ...
        // 安全获取所有需要的DOM元素
        const elements = {
            playerTerritory: document.getElementById('playerTerritory'),
            playerArmy: document.getElementById('playerArmy'),
            blueTerritory: document.getElementById('blueTerritory'),
            blueArmy: document.getElementById('blueArmy'),
            greenTerritory: document.getElementById('greenTerritory'),
            greenArmy: document.getElementById('greenArmy'),
            selectedInfo: document.getElementById('selectedInfo'),
            gameStatus: document.getElementById('gameStatus')
        };

        // 检查关键元素是否存在
        if (!elements.playerTerritory || !elements.playerArmy) {
            // 如果这是第一次发现元素缺失，尝试创建它们
            if (!window.__uiElementsCreated) {
                window.__uiElementsCreated = true;
                console.log('UI元素未找到，将尝试在下次游戏循环中创建');
            }
            return; // 跳过这次更新
        }

        // 更新玩家统计
        elements.playerTerritory.textContent = gameState.players[0].territory;
        elements.playerArmy.textContent = gameState.players[0].army;
        ...
        for (let i = 1; i < gameState.players.length; i++) {
            ...
            if (i === 1) {
                // 蓝色玩家
                if (elements.blueTerritory && elements.blueArmy) {
                    if (canSee) {
                        elements.blueTerritory.textContent = player.alive ? player.territory : '已击败';
                        elements.blueArmy.textContent = player.alive ? player.army : '已击败';
                    } else {
                        elements.blueTerritory.textContent = '?';
                        elements.blueArmy.textContent = '?';
                    }
                }
            } else if (i === 2) {
                // 绿色玩家
                if (elements.greenTerritory && elements.greenArmy) {
                    if (canSee) {
                        elements.greenTerritory.textContent = player.alive ? player.territory : '已击败';
                        elements.greenArmy.textContent = player.alive ? player.army : '已击败';
                    } else {
                        elements.greenTerritory.textContent = '?';
                        elements.greenArmy.textContent = '?';
                    }
                }
            }
        }
        ...
        if (elements.selectedInfo) {
            if (gameState.selectedTile) {
                ...
                elements.selectedInfo.textContent = `选中: 位置(${gameState.selectedTile.x},${gameState.selectedTile.y}) ${tileType} 兵力: ${tile.army}`;
            } else {
        ...
        if (elements.gameStatus) {
            const statusElement = elements.gameStatus.querySelector('.status-text');
            if (statusElement) {
